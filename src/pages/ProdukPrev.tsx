...
const ProdukDetail: React.FC = () => {
  ...
  const sliderSettings = {
    ...
    autoplaySpeed: 3000,
    arrows: true,
    accessibility: false, // ✅ cegah focus ke cloned slide
  };
          ...
          <Slider {...sliderSettings}>
            {produk.foto && produk.foto.length > 0 ? (
              produk.foto.map((img, index) => (
                <div key={index} tabIndex={-1} className="outline-none">
                  <img
                    src={img.url}
                    alt={`Foto ${index + 1}`}
                    className="w-full max-h-[300px] sm:max-h-[400px] object-cover pointer-events-none"
                    draggable={false}
                  />
                </div>
              ))
              ...
              <div
                className="w-full h-[300px] flex items-center justify-center bg-gray-200 text-gray-500 text-center px-4"
                tabIndex={-1}
              >
